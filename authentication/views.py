...
def register(request):
    if request.method == 'POST':
        ...
        # messages.add_message(request, messages.SUCCESS, 'Account create, you can now login')
        # login(request, user)
...
def login_user(request):
    if request.method == 'POST':
        ...
        if not user:
            messages.add_message(request, messages.ERROR, 'Invalid credential, try again')
            return render(request, 'authentication/login.html', context)
        login(request, user)
